...
use std::{ptr, slice};
use vpx_sys::{vp8e_enc_control_id::*, vpx_codec_err_t::*, *};

use crate::yuv::{I420ToARGB, I420ToRAW};
...
impl VpxEncoder {
    pub fn new(config: &VpxConfig) -> VpxResult<Self> {
        let iface = match config.codec {
            VpxCodec::VP8 => call_vpx_ptr!(vpx_codec_vp8_cx()),
            VpxCodec::VP9 => call_vpx_ptr!(vpx_codec_vp9_cx()),
        };

        let mut c = unsafe { MaybeUninit::zeroed().assume_init() };
        call_vpx!(vpx_codec_enc_config_default(iface, &mut c, 0));
    ...
    pub fn encode_i420(&mut self, pts: i64, data: &[u8]) -> VpxResult<EncodeFrames> {
        assert!(2 * data.len() >= 3 * self.width * self.height);

        let mut image = unsafe { zeroed() };
    ...
    pub fn flush(&mut self) -> VpxResult<EncodeFrames> {
        call_vpx!(vpx_codec_encode(
            &mut self.ctx,
            ptr::null(),
            -1, // PTS
...
impl VpxDecoder {
    ...
    /// the VP9 decoder.
    pub fn new(codec: VpxCodec) -> VpxResult<Self> {
        ...
        let iface = match codec {
            VpxCodec::VP8 => call_vpx_ptr!(vpx_codec_vp8_dx()),
            VpxCodec::VP9 => call_vpx_ptr!(vpx_codec_vp9_dx()),
        };
    ...
    pub fn decode2rgb(&mut self, data: &[u8], rgba: bool) -> VpxResult<Vec<u8>> {
        let mut img = VpxImage::new();
    ...
    pub fn decode(&mut self, data: &[u8]) -> VpxResult<DecodeFrames> {
    ...
    pub fn flush(&mut self) -> VpxResult<DecodeFrames> {
        call_vpx!(vpx_codec_decode(
            &mut self.ctx,
            ptr::null(),
            0,
